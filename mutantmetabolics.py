...
def get_values(patient_list, diseases, cursor):
    """ Deze functie haalt values uit de database en zet deze door om
    te text minen en de database vullen met pubmedIDs.
    """
    for patient in patient_list:
        for metabolite in patient.keys():
            for disease in diseases:
                compare(metabolite, disease,cursor, patient[metabolite])
                


def compare(metaboliet, disease, cursor, z_score):
...
def add_data(metaboliet, disease, pudmed_id, cursor, z_score):
    """
    :param metaboliet: -str- metaboliet
    :param disease: -str- disease van metaboliet
    :param pudmed_id: -int- getal van pubmed artikel
    """
    disease_id_query = "select id from diseases "\
        "where disease like %s"
    #cursor.execute(disease_id_query, (disease,))
    #disease_id = cursor.fetchall()[0][0]

    # metabolite_id_query = "select id from metabolite "\
    #     "where metabolite like %s"
    #cursor.execute(disease_id_query, (metaboliet,))
    #metaboliet_id = cursor.fetchall()[0][0]

    #metabolite_id_query = "select id from meta
...
def main():
    ...
    cursor = ""
    get_values(patient_z_and_meta, noduplicate_disease, cursor)
